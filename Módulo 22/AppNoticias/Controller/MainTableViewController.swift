...
class MainTableViewController: UITableViewController {
	...
	fileprivate func setUpFetchedResultController() {
		let fetchResquest:NSFetchRequest<NewsData> = NewsData.fetchRequest()
		let sortDescriptor = NSSortDescriptor(key: "title", ascending: true)
		
		fetchResquest.sortDescriptors = [sortDescriptor]
		fetchedResultController = NSFetchedResultsController(fetchRequest: fetchResquest, managedObjectContext: dataController.viewContext, sectionNameKeyPath: nil, cacheName: "appNoticias")
		fetchedResultController.delegate = self
		do {
			try fetchedResultController.performFetch()
		}catch{
			print("Sem fetchedResultController")
		}
	}
